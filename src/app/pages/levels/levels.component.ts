...
export class LevelsComponent implements OnInit {
  ...
  dragEnd($event: CdkDragEnd) {
    ...
    let screenx = this._getScreenSize().w / 2,
      screeny = this._getScreenSize().h / 2;
    if (
      screenx - aralikx < x + this.objectPosition[id - 1].x &&
      x + this.objectPosition[id - 1].x < screenx + aralikx &&
      screeny - araliky < y + this.objectPosition[id - 1].y &&
      y + this.objectPosition[id - 1].y < screeny + araliky
    ) {
      //this.changePosition(-this.objectPosition[id - 1].x,-this.objectPosition[id - 1].y,id);
      this.objectControl[id - 1] = true;
    }
    if (
      x + this.objectPosition[id - 1].x > screenx * 2 ||
      x + this.objectPosition[id - 1].x < 0 ||
      y + this.objectPosition[id - 1].y > screeny * 2 ||
      y + this.objectPosition[id - 1].y < 0
    ) {
